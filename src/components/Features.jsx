...
export const BentoTilt = ({ children, className = "" }) => {
    ...
    const handleMouseMove = (event) => {
      ...
      const newTransform = `perspective(700px) rotateX(${tiltX}deg) rotateY(${tiltY}deg) scale3d(1.0001, 1.0001, .95)`;
    ...
    <div className="relative size-full">
      ...
      <div className="relative h-full p-6 text-blue-50 z-10 flex flex-col justify-between">
        <div className="">
          ...
          {description && (
            <p className="text-sm opacity-80 mt-3 max-w-64">{description}</p>
          )}
        </div>
        <Button
          title="comming soon"
          leftIcon={<TiLocationArrow />}
          containerClass="!bg-black border border-hsla flex items-center justify-center gap-1 opacity-70"
        />
      </div>
    </div>
  );
